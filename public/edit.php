...
require __DIR__ . "/../functions/dbConnector.php"; // Connexion à la base de données
    ...
    // Si le nombre total d'enregistrement est différent de 1
    ...
    // Dans le cas contraire récupérons le film a modifier
    $film = $request->fetch();
    $request->closeCursor();
    ...
    if ( $_SERVER['REQUEST_METHOD'] === "POST" )
    {
        ...
        if ( isset($_POST['rating']) ) 
        {
            ...
            if ( trim($_POST['rating']) != "" )
            {
                ...
                {
                    $formErrors['rating'] = "La note doit être nombre.";
                }
                
                if ( $_POST['rating'] < '0' || $_POST['rating'] > '5' )
                {
                    $formErrors['rating'] = "La note doit être être comprise entre 0 et 5.";
                }
            }
        }
        ...
        $request = $db->prepare("UPDATE film SET title=:title, actors=:actors, rating=:rating, comment=:comment, updated_at=now() WHERE id=:id");
            ...
            <div class="row">
                <div class="col-md-9 col-lg-5 mx-auto p-4 shadow bg-white rounded">
                    ...
                    <form method="post">
                        ...
                        <div class="mb-3">
                            <label for="rating">La note / 5</label>
                            <input type="number" min="0" max="5" step=".1" name="rating" id="rating" class="form-control" value="<?= isset($_SESSION['old']['rating']) ? htmlspecialchars($_SESSION['old']['rating']) : htmlspecialchars($film['rating']?? ""); unset($_SESSION['old']['rating']); ?>">
                        </div>
                        <div class="mb-3">
